...
print(f"dataset path: {data_tra_path}")
...
# TODO: Add
print(f"data_tra: {data_tra_path} , \t data_val: {data_val_path}")
...
file_extension = "*.JPG"
img_names_tra = [path.name for path in Path(DIR_IMG_tra).glob(file_extension)]
mask_names_tra = [path.name for path in Path(DIR_MASK_tra).glob(file_extension)]

img_names_val = [path.name for path in Path(DIR_IMG_val).glob(file_extension)]
mask_names_val = [path.name for path in Path(DIR_MASK_val).glob(file_extension)]

train_dataset = Crack_loader(
    img_dir=DIR_IMG_tra,
    img_fnames=img_names_tra,
    mask_dir=DIR_MASK_tra,
    mask_fnames=mask_names_tra,
    isTrain=True,
)
valid_dataset = Crack_loader(
    img_dir=DIR_IMG_val,
    img_fnames=img_names_val,
    mask_dir=DIR_MASK_val,
    mask_fnames=mask_names_val,
    resize=True,
)
...
train_loader = DataLoader(
    train_dataset, batch_size=int(config["batch_size_tr"]), shuffle=True, drop_last=True
)
...
flops, params = get_model_complexity_info(
    Net, (3, 256, 256), as_strings=True, print_per_layer_stat=False
)
...
if int(config["pretrained"]):
    Net.load_state_dict(
        torch.load(config["saved_model"], map_location="cpu")["model_weights"]
    )
...
scheduler = optim.lr_scheduler.ReduceLROnPlateau(
    optimizer, "min", factor=0.5, patience=config["patience"]
)
...
for ep in range(int(config["epochs"])):
    ...
    for itter, batch in enumerate(train_loader):
        ...
        # TODO: Debug
        # print(f"img dim = {img.dim()}, msk dim: {msk.dim()}")
        # if msk.dim() == 5:
        #     print(f"msk dim == 5")
        #     # msk = msk.squeeze(-1)
        #     msk = msk.permute(0, 4, 1, 2, 3).contiguous()
        #     msk = msk.view(msk.size(0), msk.size(1), msk.size(2), msk.size(3))
        ...
        if (itter + 1) % int(float(config["progress_p"]) * len(train_loader)) == 0:
            ...
            print(
                f" Epoch>> {ep+1} and itteration {itter+1} loss>>{epoch_loss/(itter+1)}"
            )
        if (itter + 1) * int(config["batch_size_tr"]) == len(train_dataset):
            visualizer.print_current_losses(
                epoch=(ep + 1),
                iters=(itter + 1),
                loss=((epoch_loss / (itter + 1))),
                lr=lr,
                isVal=False,
            )
    ...
    with torch.no_grad():
        ...
        for itter, batch in enumerate(val_loader):
            ...
            val_loss += loss.item()
        visualizer.print_current_losses(
            epoch=ep + 1, loss=(abs(val_loss / (itter + 1))), isVal=True
        )
        ...
        if mean_val_loss < best_val_loss:
            ...
            with open(log_name, "a") as log_file:
                ...
                log_file.write("%s\n" % message)
            state = copy.deepcopy(
                {"model_weights": Net.state_dict(), "val_loss": best_val_loss}
            )
